...
# Format columns.    
    ...
    cgmupload["date_cgm_placement", f] <- as.character(as.Date(table$subjectid[2],format = "%m/%d/%Y %T"))
    table$subjectid[2] <- base::as.POSIXct(lubridate::parse_date_time(table$subjectid[2],dateparseorder),tz = "UTC")
    table$subjectid[3] <- base::as.POSIXct(lubridate::parse_date_time(table$subjectid[3],dateparseorder),tz = "UTC")
    
    totaltime <- as.numeric(table$subjectid[3])-as.numeric(table$subjectid[2])
    cgmupload["percent_cgm_wear",f] <- round(((length(table$sensorglucose)/(totaltime/interval))*100),2)
...
# Excursions over 120 calculations.
    BGover120 <- base::as.numeric(table$sensorglucose[base::which(!is.na(table$sensorglucose))],length = 1)
    BGover120[BGover120 < 120] <- 0
    BGover120[BGover120 >= 120] <- 1
    BG120.rle <- base::rle(BGover120)
    excursions120 <- base::as.numeric(BG120.rle$lengths[base::which(BG120.rle$values == 1)])
    ...
    cgmupload["percent_time_over_120",f] <- 
      ((base::sum(BGover120) * (interval/60))/(base::length(table$sensorglucose) * 
                                           (interval/60))) * 100
...
# Over 140.
    ...
    excursions140 <- base::as.numeric(BG140.rle$lengths[base::which(BG140.rle$values == 1)])
    ...
    cgmupload["percent_time_over_140",f] <- 
      ((base::sum(BGover140) * (interval/60))/(base::length(table$sensorglucose) * 
                                           (interval/60))) * 100
...
# Over 180.
    ...
    excursions180 <- base::as.numeric(BG180.rle$lengths[base::which(BG180.rle$values == 1)])
    ...
    cgmupload["percent_time_over_180",f] <- 
      ((base::sum(BGover180) * (interval/60))/(base::length(table$sensorglucose) * 
                                           (interval/60))) * 100
...
# Over 200.
    ...
    excursions200 <- base::as.numeric(BG200.rle$lengths[base::which(BG200.rle$values == 1)])
    ...
    cgmupload["percent_time_over_200",f] <- 
      ((base::sum(BGover200) * (interval/60))/(base::length(table$sensorglucose) * 
                                           (interval/60))) * 100

    cgmupload["avg_excur_over_140_per_day",f] <- as.numeric(cgmupload["excursions_over_140",f])/as.numeric(cgmupload["num_days_good_data",f])
    cgmupload["avg_excur_over_200_per_day",f] <- as.numeric(cgmupload["excursions_over_200",f])/as.numeric(cgmupload["num_days_good_data",f])
...
# Over 250.
    ...
    excursions250 <- base::as.numeric(BG250.rle$lengths[base::which(BG250.rle$values == 1)])
    ...
    cgmupload["percent_time_over_250",f] <- 
      ((base::sum(BGover250) * (interval/60))/(base::length(table$sensorglucose) * 
                                           (interval/60))) * 100
...
# Under 54.
    ...
    excursions54 <- base::as.numeric(BG54.rle$lengths[base::which(BG54.rle$values == 1)])
    ...
    cgmupload["percent_time_under_54",f] <- 
      ((base::sum(BGunder54) * (interval/60))/(base::length(table$sensorglucose) * 
                                           (interval/60))) * 100
...
# Under 60.
    ...
    excursions60 <- base::as.numeric(BG60.rle$lengths[base::which(BG60.rle$values == 1)])
    ...
    cgmupload["percent_time_under_60",f] <- 
      ((base::sum(BGunder60) * (interval/60))/(base::length(table$sensorglucose) * 
                                           (interval/60))) * 100
...
# Under 70.
    ...
    excursions70 <- base::as.numeric(BG70.rle$lengths[base::which(BG70.rle$values == 1)])
    ...
    cgmupload["percent_time_under_70",f] <- 
      ((base::sum(BGunder70) * (interval/60))/(base::length(table$sensorglucose) * 
                                           (interval/60))) * 100
    
# Find daytime AUC.
    daytime_indexes <- base::which(base::as.numeric(base::format(table$timestamp,"%H")) %in% 6:22)
...
# Day/night ratio.
    cgmupload["day_night_sensor_ratio",f] <- round(length(daytime_sensor)/length(nighttime_sensor),1)
...
# Other nighttime sensor glucose variables.
    cgmupload["nighttime_avg_sens_glucose",f] <- base::mean(na.omit(nighttime_sensor))
...
# width = minutes.
    if (base::length(sensorover180) > 1) {
      ...
      aucs <- (aucs[base::length(sensorover180)]) - (xaxis[base::length(xaxis)] * 180)
...
# unsmoothed data.  
    table$smoothed <- base::as.numeric(zoo::rollapply(zoo::zoo(table$sensorglucose), 9, function(x) c(1,2,4,8,16,8,4,2,1) %*% (x / 46),fill = NA))
...
# Calculate the average of the differences greater than the entire dataset SD, 2SD, etc.
    if (magedef == "1sd") {
      cgmupload["r_mage",f] <- base::mean(na.omit(differences[base::which(differences > sd)]))
    } else if (magedef == "1.5sd") {
      cgmupload["r_mage",f] <- base::mean(na.omit(differences[base::which(differences > (sd * 1.5))]))
    } else if ( magedef == "2sd") {
      cgmupload["r_mage",f] <- base::mean(na.omit(differences[base::which(differences > (sd * 2))]))
    } else {
      cgmupload["r_mage",f] <- base::mean(na.omit(differences[base::which(differences > magedef)]))
    }
